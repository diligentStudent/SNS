...
export default {
  ...
  methods: {
    ...
    infiniteHandler($state){
      let ref=this;
      axios.post('https://i3b304.p.sns.io/api/recell/all/'+ref.limit)
      .then((data)=>{
        setTimeout(() => {
    ...
    getAllList() {
      axios.post("https://i3b304.p.sns.io/api/recell/newsfeed/0").then((data)=>{
    ...
    getMyList() {
      axios.get("https://i3b304.p.sns.io/api/recell/myContents",{
        params: {
    ...
    soldItem(roomNo) {
      Swal.fire({
        ...
        if (result.value) {
          ...
          axios.post('https://i3b304.p.sns.io/api/recell/soldout', frm)
          .then()
          .catch()
          Swal.fire(
            '완료되었습니다.',
